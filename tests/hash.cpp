...
TEST(Hash, HashOfValuelessObjectIsReproducible)
{
  ...
  EXPECT_EQ(std::hash<xyz::copy_on_write<int>>{}(a), std::hash<xyz::copy_on_write<int>>{}(a));
}
...
TEST(Hash, TwoObjectsWithDifferentValuesProduceDifferentHashes)
{
  ...
  EXPECT_NE(std::hash<xyz::copy_on_write<int>>{}(a), std::hash<xyz::copy_on_write<int>>{}(b));
}
